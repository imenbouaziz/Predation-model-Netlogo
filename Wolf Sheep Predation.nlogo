...
patches-own [ countdown deadly-weed? ]    ; this is for the sheep-wolves-grass model version
...
to setup
  ...
  ;set werewolf-reproduce 50
  ;set werewolf-gain-from-wolf 20 ; Adjust as needed
  ...
  [
  set shape "wolf"  ; You can choose a different shape for red wolves if needed
  set color red
  set size 4
  set energy random (2 * wolf-gain-from-food)
  setxy random-xcor random-ycor
  ]
  create-werewolves initial-number-warewolves
  [
  set shape "wolf"  ; You can choose a different shape for werewolves if needed
  set color orange  ; Choose a color for werewolves
  set size 2
  set energy random (2 * wolf-gain-from-food)
  setxy random-xcor random-ycor
  ]

  create-vampires initial-number-vampires
  [
  set shape "x"  ; You can choose a different shape for vampires if needed
  set color blue  ; Choose a color for vampires
  set size 2
  set energy random (2 * vampire-gain-from-wolf-sheep)
  setxy random-xcor random-ycor
  ]

  ; Mark patches as deadly weed
  ask n-of initial-number-deadly-weed patches [
    set pcolor violet ; Setting the color of deadly weed
    set deadly-weed? true
  ]
...
to go
  ...
  ask sheep [
    ...
    if model-version = "sheep-wolves-grass" [
      ...
      ; Check if on a deadly weed patch and reduce energy
...
to eat-grass  ; sheep procedure
  ...
  if pcolor = violet [
    set pcolor brown
    set energy energy / 2; sheep lose energy , deadly weed
  ]

  ; Deduct energy whether or not grass is eaten
  set energy energy - 1
end
...
to reproduce-werewolves  ; werewolf procedure
  if random-float 100 < werewolf-reproduce [
    set energy (energy / 2)
    hatch 1 [ rt random-float 360 fd 1 ]
  ]
end

to reproduce-vampires  ; vampire procedure
  if random-float 100 < vampire-reproduce [
    set energy (energy / 2)
    hatch 1 [ rt random-float 360 fd 1 ]
  ]
end
...
to eat-wolves  ; werewolf procedure
  let prey one-of wolves-here
  if prey != nobody  [
    ask prey [ die ]
    set energy energy + werewolf-gain-from-wolf
  ]
end

to eat-wolves-and-sheep  ; vampire procedure
  let prey one-of turtles-here with [breed = wolves or breed = sheep]
  if prey != nobody  [
    ask prey [ die ]
    set energy energy + vampire-gain-from-wolf-sheep
  ]
end
...
"werewolves" 1.0 0 -955883 true "" "plot count werewolves"
"vampires" 1.0 0 -13345367 true "" "plot count vampires"
